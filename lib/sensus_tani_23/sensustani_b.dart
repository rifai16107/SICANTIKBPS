...
class SensusTaniB extends StatefulWidget {
  const SensusTaniB({Key? key}) : super(key: key);

  @override
  State<SensusTaniB> createState() => _SensusTaniBState();
}

RepositorySensusTani repositorysensustani = RepositorySensusTani();

class _SensusTaniBState extends State<SensusTaniB> {
   @override
        ...
        if (snapshot.hasData) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              int rtup = int.parse(isisensustani[index = 1].rtup);
              int panganRtup = int.parse(isisensustani[index = 1].padi_rtup);
              int hortiRtup = int.parse(isisensustani[index = 1].horti_rtup);
              int kebunRtup = int.parse(isisensustani[index = 1].kebun_rtup);
              int ternakRtup = int.parse(isisensustani[index = 1].ternak_rtup);
              int ikanRtup = int.parse(isisensustani[index = 1].ikan_rtup);
              int hutanRtup = int.parse(isisensustani[index = 1].hutan_rtup);
              int jasaRtup = int.parse(isisensustani[index = 1].jasa_rtup);
              ...
              int utp = int.parse(isisensustani[index = 1].utp);
              int panganUtp = int.parse(isisensustani[index = 1].padi_utp);
              int hortiUtp = int.parse(isisensustani[index = 1].horti_utp);
              int kebunUtp = int.parse(isisensustani[index = 1].kebun_utp);
              int ternakUtp = int.parse(isisensustani[index = 1].ternak_utp);
              int ikanUtp = int.parse(isisensustani[index = 1].ikan_utp);
              int hutanUtp = int.parse(isisensustani[index = 1].hutan_utp);
              int jasaUtp = int.parse(isisensustani[index = 1].jasa_utp);
                ...
                child: Column(
                  children: <Widget>[
                    Container(
                      ...
                      child: Center(
                        child: Text(
                          'Sensus Pertanian $thn (ST$thn) mencatat jumlah Rumah Tangga Usaha Pertanian (RTUP) dan Jumlah Usaha Pertanian Perorangan (UTP) di Provinsi Jawa Tengah sebanyak : ',
