...
class GrafikDistPdrbAdhkMigas extends StatefulWidget {
  ...
  @override
  State<GrafikDistPdrbAdhkMigas> createState() =>
      _GrafikDistPdrbAdhkMigasState();
}
        ...
        if (snapshot.hasData) {
          ...
          return PageView.builder(
            ...
            itemBuilder: (context, index) {
              double a = double.parse(isipdrb[index = 0].a);
              double b = double.parse(isipdrb[index = 0].b);
              double c = double.parse(isipdrb[index = 0].c);
              double d = double.parse(isipdrb[index = 0].d);
              double e = double.parse(isipdrb[index = 0].e);
              double f = double.parse(isipdrb[index = 0].f);
              double g = double.parse(isipdrb[index = 0].g);
              double h = double.parse(isipdrb[index = 0].h);
              double i = double.parse(isipdrb[index = 0].i);
              double j = double.parse(isipdrb[index = 0].k);
              double k = double.parse(isipdrb[index = 0].k);
              double l = double.parse(isipdrb[index = 0].l);
              double mn = double.parse(isipdrb[index = 0].m_n);
              double o = double.parse(isipdrb[index = 0].o);
              double p = double.parse(isipdrb[index = 0].p);
              double q = double.parse(isipdrb[index = 0].q);
              double rstu = double.parse(isipdrb[index = 0].r_s_t_u);

              String th1 = (isipdrb[index = 0].created_at[0] +
              ...
              return SizedBox(
                    ...
                    child: SfCartesianChart(
                        title: ChartTitle(
                        text: 'Sumber Pertumbuhan PDRB ADHK Kabupaten Cilacap Menurut Lapangan Usaha -Dengan Migas (Persen) Tahun $th1',
